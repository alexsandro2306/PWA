...
const Sidebar = () => {
    ...
    const menuItems = [
        ...
        { icon: <FileText size={20} />, label: 'Planos de Treino', path: '/trainer/plans-management', roles: ['trainer'], end: true },
    ...
    const filteredMenu = menuItems.filter(item => {
        ...
        if (user?.role === 'trainer' && !user.isValidated) {
            if (['/trainer/clients', '/trainer/plans', '/trainer/exercises', '/chat', '/'].includes(item.path)) return false;
        }
        return true;
    });
