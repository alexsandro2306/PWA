...
const ClientWorkouts = () => {
  ...
  const [showMissedReason, setShowMissedReason] = useState(false);
  ...
  const fetchData = async () => {
    try {
      ...
      try {
        ...
        // Don't show error for logs, just set empty array
        setTrainingLogs([]);
      }
  ...
  const getWorkoutForDay = (date) => {
    ...
    for (const plan of trainingPlans) {
      ...
      if (!plan.weeklyPlan || !Array.isArray(plan.weeklyPlan)) continue;

      const startDate = new Date(plan.startDate);
      const endDate = new Date(plan.endDate);

      if (date >= startDate && date <= endDate) {
        const workout = plan.weeklyPlan.find(w => w.dayOfWeek === dayOfWeek);
  ...
  const markWorkout = async (date, isCompleted, reason = '') => {
    try {
      ...
      const response = await api.post('/workouts/client/logs', {
        planId: workout.plan._id,
        workoutId: workout.workout.dayOfWeek, // Use dayOfWeek as identifier
      ...
      // Check if it's a duplicate error (which means it was actually saved)
      if (error.response?.data?.message?.includes('Já existe um registo')) {
        addNotification('warning', 'Este treino já foi registado anteriormente');
        await fetchData(); // Refresh to show the existing log
        setSelectedDay(null);
        setShowMissedReason(false);
        setMissedReason('');
      } else {
        addNotification('error', error.response?.data?.message || 'Erro ao marcar treino');
      }
    }
  };
  ...
  return (
    <div className="workouts-page">
                      ...
                      <>
                        {!showMissedReason ? (
                          <div className="modal-actions">
                            ...
                            <button
                              onClick={() => setShowMissedReason(true)}
                          ...
                          <div className="missed-reason-form">
                            <label htmlFor="missed-reason">Motivo para não realizar o treino:</label>
                            <textarea
                              id="missed-reason"
                              value={missedReason}
                              onChange={(e) => setMissedReason(e.target.value)}
                              placeholder="Ex: Lesão, falta de tempo, doença..."
                              rows={3}
                              autoFocus
                            />
                            <div className="missed-reason-actions">
                              <button
                                onClick={() => {
                                  if (missedReason.trim()) {
                                    markWorkout(selectedDay, false, missedReason);
                                  } else {
                                    addNotification('error', 'Por favor, indica o motivo');
                                  }
                                }}
                                className="btn-primary"
      ...
      <style>{`
        ...
        .missed-reason-form label {
          font-size: 0.875rem;
          font-weight: 600;
          color: var(--text-primary);
        }

        .missed-reason-form textarea {
          width: 100%;
          padding: 0.75rem;
          border: 1px solid var(--border-color);
          border-radius: 0.5rem;
        ...
        .missed-reason-form textarea:focus {
          outline: none;
          border-color: var(--accent-primary);
          box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1);
        }

        .missed-reason-form textarea::placeholder {
          color: var(--text-secondary);
          opacity: 0.6;
        }

        .missed-reason-actions {
          display: grid;
          grid-template-columns: 1fr 1fr;
          gap: 0.75rem;
        }
